...
The exact NS6711 build also reports `com.amazon.ftvads.deeplinking` and
`com.amazon.tv.csapp` as protected packages when `pm disable-user --user 0` is
attempted. They are mandatory core preserve entries and must remain enabled;
neither is a successful or supported debloat item on this model/build.
